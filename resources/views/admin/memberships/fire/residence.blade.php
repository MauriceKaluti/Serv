...
<head>
    ...
    <title>ResQ 247 | Residence Emergency Fire Applications </title>
    <!-- Favicon-->
      ...
      <section class="content">
        <div class="container-fluid">
            <div class="block-header">
                <h2>
                    Residence Emergency Fire Applications
                    <small>Explore All ResQ 247 Residence Emergency Fire Applications</a></small>
                </h2>
            </div>
                ...
                <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                    <div class="card">
                      ...
                      <!-- Notification -->
                        ...
                        <strong>Oh Snap! Residence Emergency Fire Application Deleted Successfully!</strong>  </div>
                         @endif
                    <!-- End Notification -->
                     ...
                     @forelse($resDs as $resD)
                        <tr>
                        <td>{{$resD->id}}</td>
                        <td>{{$resD->package_type}}</td>                     
                        <td>{{$resD->res_name}}</td>                     
                        <td>{{$resD->email}}</td> 
                    
                        <td>{{$resD->phone_one}}</td> 
                      
                        <td>{{$resD->serv_category}}</td> 
                        <td>
                           <li>{{$resD->fire_engine}}</li>
                           <li>{{$resD->fire_marshal}}</li> 
                           <li>{{$resD->service_equip}}</li>
                           <li>{{$resD->supply_service}}</li>
                           <li>{{$resD->fire_safety}}</li>
                        </td>
                        <td>{{$resD->cost}}</td>                                         
                        <td>{{$resD->created_at}}</td>         

                         <td>
                            <form action="{{route('deletefireResidence',$resD->id)}}" method="post">
                                {{csrf_field()}}
                             <button style="border-style: none;"  type="submit"  title="Delete Residence Application Forever"> <i style="color: red;" class="fa fa-trash"></i>
                            </button>&nbsp&nbsp
