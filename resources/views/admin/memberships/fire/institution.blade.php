...
<head>
    ...
    <title>ResQ 247 | Institution Emergency Fire Applications </title>
    <!-- Favicon-->
      ...
      <section class="content">
        <div class="container-fluid">
            <div class="block-header">
                <h2>
                    Institution Emergency Fire Applications
                    <small>Explore All ResQ 247 Institution Emergency Fire Applications</a></small>
                </h2>
            </div>
                ...
                <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                    <div class="card">
                      ...
                      <!-- Notification -->
                        ...
                        <strong>Oh Snap! Institution Emergency Fire Application Deleted Successfully!</strong>  </div>
                         @endif
                    <!-- End Notification -->
                     ...
                     @forelse($insts as $inst)
                        <tr>
                        <td>{{$inst->id}}</td>
                        <td>{{$inst->package_type}}</td>                     
                        <td>{{$inst->inst_name}}</td>                     
                        <td>{{$inst->email}}</td> 
                    
                        <td>{{$inst->phone_one}}</td> 
                        <td>{{$inst->number_students}}</td> 
                        <td>{{$inst->serv_category}}</td> 
                        <td>
                           <li>{{$inst->fire_engine}}</li>
                           <li>{{$inst->fire_marshal}}</li> 
                           <li>{{$inst->service_equip}}</li>
                           <li>{{$inst->supply_service}}</li>
                           <li>{{$inst->fire_safety}}</li>
                        </td>
                        <td>{{$inst->cost}}</td>                                         
                        <td>{{$inst->created_at}}</td>         

                         <td>
                            <form action="{{route('deletefireInstitution',$inst->id)}}" method="post">
                                {{csrf_field()}}
                             <button style="border-style: none;"  type="submit"  title="Delete Institution Application Forever"> <i style="color: red;" class="fa fa-trash"></i>
