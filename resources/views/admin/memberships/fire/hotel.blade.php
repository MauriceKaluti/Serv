...
<head>
    ...
    <title>ResQ 247 | Hotel Emergency Fire Applications </title>
    <!-- Favicon-->
      ...
      <section class="content">
        <div class="container-fluid">
            <div class="block-header">
                <h2>
                    Hotel Emergency Fire Applications
                    <small>Explore All ResQ 247 Hotel Emergency Fire Applications</a></small>
                </h2>
            </div>
                ...
                <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                    <div class="card">
                      ...
                      <!-- Notification -->
                        ...
                        <strong>Oh Snap! Hotel Emergency Fire Application Deleted Successfully!</strong>  </div>
                         @endif
                    <!-- End Notification -->
                     ...
                     @forelse($hotels as $hotel)
                        ...
                        <td>{{$hotel->phone_one}}</td> 
                        <td>{{$hotel->number_staffs}}</td> 
                      
                        <td>{{$hotel->serv_category}}</td> 
                        <td>
                           <li>{{$hotel->fire_engine}}</li>
                           <li>{{$hotel->fire_marshal}}</li> 
                           <li>{{$hotel->service_equip}}</li>
                           <li>{{$hotel->supply_service}}</li>
                           <li>{{$hotel->fire_safety}}</li>
                        </td>
                        <td>{{$hotel->cost}}</td>                                         
                        <td>{{$hotel->created_at}}</td>         

                         <td>
                            <form action="{{route('deletefireHotel',$hotel->id)}}" method="post">
                                {{csrf_field()}}
                             <button style="border-style: none;"  type="submit"  title="Delete Hotel Application Forever"> <i style="color: red;" class="fa fa-trash"></i>
                            </button>&nbsp&nbsp
