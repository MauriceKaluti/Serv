...
<head>
    ...
    <title>ResQ 247 | Corporate Emergency Fire Applications </title>
    <!-- Favicon-->
      ...
      <section class="content">
        <div class="container-fluid">
            <div class="block-header">
                <h2>
                    Corporate Emergency Fire Applications
                    <small>Explore All ResQ 247 Corporate Emergency Fire Applications</a></small>
                </h2>
            </div>
                ...
                <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                    <div class="card">
                      ...
                      <!-- Notification -->
                        ...
                        <strong>Oh Snap! Corporate Emergency Fire Application Deleted Successfully!</strong>  </div>
                         @endif
                    <!-- End Notification -->
                     ...
                     @forelse($corps as $corp)
                        <tr>
                        <td>{{$corp->id}}</td>
                        <td>{{$corp->package_type}}</td>                     
                        <td>{{$corp->org_name}}</td>                     
                        <td>{{$corp->email}}</td> 
                    
                        <td>{{$corp->phone_one}}</td> 
                      
                        <td>{{$corp->serv_category}}</td> 
                        <td>
                           <li>{{$corp->fire_engine}}</li>
                           <li>{{$corp->fire_marshal}}</li> 
                           <li>{{$corp->service_equip}}</li>
                           <li>{{$corp->supply_service}}</li>
                           <li>{{$corp->fire_safety}}</li>
                        </td>
                        <td>{{$corp->cost}}</td>                                         
                        <td>{{$corp->created_at}}</td>         

                         <td>
                            <form action="{{route('deletefireCorporate',$corp->id)}}" method="post">
                                {{csrf_field()}}
                             <button style="border-style: none;"  type="submit"  title="Delete Corporate Application Forever"> <i style="color: red;" class="fa fa-trash"></i>
                            </button>&nbsp&nbsp
