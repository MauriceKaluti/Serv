...
<head>
    ...
    <title>ResQ 247 | Individual Emergency Medical Applications </title>
    <!-- Favicon-->
      ...
      <section class="content">
        <div class="container-fluid">
            <div class="block-header">
                <h2>
                    Individual Emergency Medical Applications
                    <small>Explore All ResQ 247 Individual Emergency Medical Applications</a></small>
                </h2>
            </div>
                ...
                <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                    <div class="card">
                      ...
                      <!-- Notification -->
                        ...
                        <strong>Oh Snap! Individual Emergency Medical Application Deleted Successfully!</strong>  </div>
                         @endif
                    <!-- End Notification -->
                     ...
                     @forelse($individuals as $individual)
                        <tr>
                        <td>{{$individual->id}}</td>
                        <td>{{$individual->ind_name}}</td>                     
                        <td>{{$individual->email}}</td>                    
                        <td>{{$individual->phone_one}}</td>               
                        <td>{{$individual->town}}</td>               
                        <td>{{$individual->refer_facility}}</td>               
                        <td>{{$individual->special_condition}}</td>               
                        <td>{{$individual->emergency_med_ind_price}}</td>                                         
                        <td>{{$individual->created_at}}</td>         

                         <td>
                            <form action="{{route('deletemedicalIndividual',$individual->id)}}" method="post">
                                {{csrf_field()}}
                             <button style="border-style: none;"  type="submit"  title="Delete Individual Application Forever"> <i style="color: red;" class="fa fa-trash"></i>
                            </button>&nbsp&nbsp
