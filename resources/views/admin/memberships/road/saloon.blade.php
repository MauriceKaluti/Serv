...
<head>
    ...
    <title>ResQ 247 | Saloon Emergency Road Applications </title>
    <!-- Favicon-->
      ...
      <section class="content">
        <div class="container-fluid">
            <div class="block-header">
                <h2>
                    Saloon Emergency Road Applications
                    <small>Explore All ResQ 247 Saloon Emergency Road Applications</a></small>
                </h2>
            </div>
                ...
                <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                    <div class="card">
                      ...
                      <!-- Notification -->
                        ...
                        <strong>Oh Snap! Saloon Emergency Road Application Deleted Successfully!</strong>  </div>
                         @endif
                    <!-- End Notification -->
                     ...
                     @forelse($saloons as $saloon)
                        <tr>
                        <td>{{$saloon->id}}</td>
                        <td>{{$saloon->owner_name}}</td>                     
                        <td>{{$saloon->email}}</td> 
                        <td>{{$saloon->phone}}</td> 
                        <td>{{$saloon->vehicle_model}}</td> 
                        <td>{{$saloon->town}}</td> 
                        <td>{{$saloon->road_price}}</td>                                         
                        <td>{{$saloon->created_at}}</td>         

                         <td>
                            <form action="{{route('deleteroadSaloon',$saloon->id)}}" method="post">
                                {{csrf_field()}}
                             <button style="border-style: none;"  type="submit"  title="Delete Saloon Application Forever"> <i style="color: red;" class="fa fa-trash"></i>
