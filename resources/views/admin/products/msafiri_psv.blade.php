...
<head>
    ...
    <title>ResQ 247 | Msafiri PSV Subscriptions </title>
    <!-- Favicon-->
      ...
      <section class="content">
        <div class="container-fluid">
            <div class="block-header">
                <h2>
                    Msafiri PSV Subscriptions
                    <small>Explore All ResQ 247 Msafiri PSV Subscriptions</a></small>
                </h2>
            </div>
                ...
                <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                    <div class="card">
                      ...
                      <!-- Notification -->
                        ...
                        <strong>Oh Snap! Msafiri PSV Subscription Deleted Successfully!</strong>  </div>
                         @endif
                    <!-- End Notification -->
                     ...
                     @forelse($msafpsvs as $msafpsv)
                        <tr>
                        <td>{{$msafpsv->id}}</td>
                        <td>{{$msafpsv->sacco_name}}</td>
                        <td>{{$msafpsv->route_name}}</td>
                        <td>{{$msafpsv->fleet_no}}</td>
                        <td>{{$msafpsv->booking_office}}</td>                                          
                        <td>{{$msafpsv->county}}</td>
                        <td>{{$msafpsv->town}}</td> 
                        <td>{{$msafpsv->booking_person}}</td> 
                        <td>{{$msafpsv->departure_time}}</td> 
                        <td>{{$msafpsv->vehicle_reg}}</td> 
                        <td>{{$msafpsv->vehicle_driver}}</td>
                        <td>{{$msafpsv->psv_price}}</td>                                         
                        <td>{{$msafpsv->created_at}}</td>         

                         <td>
                            <form action="{{route('deleteMsafiriPsv',$msafpsv->id)}}" method="post">
                                {{csrf_field()}}
                             <button style="border-style: none;"  type="submit"  title="Delete Msafiri Subscription Forever"> <i style="color: red;" class="fa fa-trash"></i>
