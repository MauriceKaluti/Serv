...
<head>
    ...
    <title>ResQ 247 | Fanaka Subscriptions </title>
    <!-- Favicon-->
      ...
      <section class="content">
        <div class="container-fluid">
            <div class="block-header">
                <h2>
                    Fanaka Subscriptions
                    <small>Explore All ResQ 247 Fanaka Subscriptions</a></small>
                </h2>
            </div>
                ...
                <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                    <div class="card">
                      ...
                      <!-- Notification -->
                        ...
                        <strong>Oh Snap! Fanaka Subscription Deleted Successfully!</strong>  </div>
                         @endif
                    <!-- End Notification -->
                     ...
                     @forelse($fanakas as $fanaka)
                        <tr>
                        <td>{{$fanaka->id}}</td>
                        <td>{{$fanaka->user_name}}</td>                     
                        <td>{{$fanaka->email}}</td> 
                        <td>{{$fanaka->phone}}</td> 
                        <td>{{$fanaka->vehicle_model}}</td> 
                        <td>{{$fanaka->town}}</td> 
                        <td>{{$fanaka->fanaka_price}}</td>                                         
                        <td>{{$fanaka->created_at}}</td>         

                         <td>
                            <form action="{{route('deleteFanaka',$fanaka->id)}}" method="post">
                                {{csrf_field()}}
                             <button style="border-style: none;"  type="submit"  title="Delete Fanaka Subscription Forever"> <i style="color: red;" class="fa fa-trash"></i>
