...
<head>
    ...
    <title>ResQ 247 | Emergency Air Quotes </title>
    <!-- Favicon-->
      ...
      <section class="content">
        <div class="container-fluid">
            <div class="block-header">
                <h2>
                    All Air Quotes
                    <small>Explore All ResQ 247 Air Quotes</a></small>
                </h2>
            </div>
                ...
                <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                    <div class="card">
                      ...
                      <!-- Notification -->
                        ...
                        <strong>Oh Snap! Air Quote Deleted Successfully!</strong>  </div>
                         @endif
                    <!-- End Notification -->
                     ...
                     @forelse($airQuotes as $airQuote)
                        <tr>
                        <td>{{$airQuote->id}}</td>
                        <td>{{$airQuote->name}}</td>                 
                        <td>{{$airQuote->email}}</td>
                        <td>{{$airQuote->phone}}</td>
                        <td>{{$airQuote->origin_location}}</td>                                      
                        <td>{{$airQuote->facility_dest}}</td>                                      
                        <td>{{$airQuote->nature_patient}}</td>                                      
                        <td>{{$airQuote->created_at}}</td>         

                         <td>
                            <form action="{{route('deleteQuote',$airQuote->id)}}" method="post">
                                {{csrf_field()}}
                             <button style="border-style: none;"  type="submit"  title="Delete Quote Forever"> <i style="color: red;" class="fa fa-trash"></i>

                            </button>&nbsp&nbsp
                            <!-- <a title="Details" href="#" title="See More Details"><i class="fa fa-eye"></i></a>  -->
                         </form>
                         </td>
                        </tr>
